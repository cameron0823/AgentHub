...
test.describe("MCP Client", () => {
  test("user configures an HTTP MCP server in settings", async ({ page }) => {
    ...
    await page
      .locator("select")
      .filter({ has: page.locator("option", { hasText: "HTTP" }) })
      .selectOption("http");
    await page.getByPlaceholder("http://localhost:3001").fill("http://localhost:3999");
    await page.getByRole("button", { name: "Add", exact: true }).click();

    const serverRow = page.getByTestId("mcp-server-row").filter({ hasText: serverName });
    await expect(serverRow).toBeVisible({ timeout: 15_000 });
    await expect(serverRow.getByText("http", { exact: true })).toBeVisible();
  });
