...
import { FolderOpen, Plus, FileText, Loader2, Search, Upload, X } from "lucide-react";
import { trpc } from "@/lib/trpc";

const SUPPORTED_UPLOAD_ACCEPT = ".pdf,.docx,.csv,.xlsx,.vtt,.srt,.md,.markdown,.ts,.tsx,.js,.jsx,.py,.json";
...
export function KnowledgeBaseManager() {
  ...
  const documents = trpc.knowledgeBases.documents.useQuery({ knowledgeBaseId: selectedKb! }, { enabled: !!selectedKb });
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 3. Read file content for text-like formats; binary formats are parsed server-side.
      let content: string | undefined;
      if (file.type.startsWith("text/") || /\.(md|markdown|csv|vtt|srt|ts|tsx|js|jsx|py|json)$/i.test(file.name)) {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8" data-testid="knowledge-base">
      ...
      {showCreate && (
        <div className="agenthub-glass-panel mb-6 space-y-3 rounded-2xl p-4">
          <input
            value={newKbName}
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowCreate(false)} className="agenthub-secondary-button">
              Cancel
            </button>
          </div>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="space-y-4 md:col-span-2">
          {activeKb ? (
            <>
              <div className="agenthub-glass-panel rounded-2xl p-4">
                ...
                <div className="text-sm text-muted-foreground mb-4">
                  Embedding: {activeKb.embeddingModel} · Chunk size: {activeKb.chunkSize} · Overlap:{" "}
                  {activeKb.chunkOverlap}
                </div>
                ...
                <div className="flex gap-2 mb-4">
                  <input
                    ...
                    className="agenthub-field flex-1 px-3 py-2 text-sm"
                    onKeyDown={(e) => {
                ...
                {queryResults.length > 0 && (
                  <div className="space-y-2 mb-4">
                    ...
                    {queryResults.map((result) => (
                      <div key={result.id} className="rounded-xl bg-white/10 p-2 text-sm">
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <button
                    ...
                    disabled={uploading}
                    className="agenthub-secondary-button disabled:opacity-50"
                  >
                    {uploading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Upload className="w-4 h-4" />}
                    {uploading ? "Uploading..." : "Upload Document"}
                  </button>
                  <span className="text-xs text-muted-foreground">
                    Supported: PDF, DOCX, CSV, XLSX, transcripts, code, and Markdown.
                  </span>
                </div>
                ...
                {documents.data?.length === 0 ? (
                  <div className="rounded-xl border border-dashed border-white/15 bg-white/5 py-4 text-center text-sm text-muted-foreground">
                    No documents yet. Upload files to get started.
                  </div>
                ) : (
                  <div className="space-y-1">
                    {documents.data?.map((doc) => (
                      <div key={doc.id} className="flex items-center gap-2 rounded-xl p-2 text-sm hover:bg-white/10">
            ...
            <div className="agenthub-glass-panel rounded-2xl py-12 text-center text-muted-foreground">
